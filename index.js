...
var retryCount = 20;
var waitForRetry = 30000; // 30sec before retry

function sendCommand(cmd) {
    // send a command to lightify
    commands.push(cmd);
    if (commands.length == 1) {
        client.write(cmd.buffer);
    } //RV changed and added
    ...
    return new Promise(function (resolve, reject) {
        ...
        client.on('data', function (data) {
            ...
            for (var i = 0; i < commands.length; i++) {
                ...
                if (commands.length > 0) {
                    client.write(commands[0].buffer);
                }
            }
        });
        ...
        client.on('error', function (error) {
            if ((error.code == 'ECONNREFUSED') || (error.code == 'ECONNRESET')) {
                if (retryCount >= 0) {
                    client.setTimeout(waitForRetry, function () {
                        retryCount -= 1;
                        console.log('Remaining Retry Count: ' + retryCount);
                        client.connect(4000, ip, function () {
                            console.log('CONNECTED Again');
                        });
                    });
                    console.log('Timeout for 30 seconds before trying again');
                }
            }
            if (onError && retryCount < 0) { // send error to caller
                onError(error);
            }
            client.on('end', function () {
                console.log("Client received end");
                //if (onError) { // send end
                //    onError('End Received');
                //}
            });
            client.on('close', function (had_error) {
                console.log("Client closed. Has Error: " + had_error);
                if (onError && had_error) { // send error
                    onError(had_error);
                }
            });
            client.connect(4000, ip, function () {
...
var exports = module.exports = {
    ...
    isBrightnessSupported: function (type) {
        return getNodeType(type) === 2 || getNodeType(type) === 4 || (getNodeType(type) != 16 && getNodeType(type) != 1);
    },
